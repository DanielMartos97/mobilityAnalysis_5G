...
phone_cells = get_cells_from_opensignal(bbox_cells); %Conector que obtiene las coordenadas de las ubicaciones reales de los puntos de acceso existentes para LTE
...
if isempty(filename)
    ...
    rx_coordinates=xlsread(filename);
    receivers = rxsite(...
        'Latitude', rx_coordinates(1,1), ...
        'Longitude', rx_coordinates(1,2), ...
        'AntennaHeight', 1.5);
end
...
filehandle = java.io.File('Scenes/mainScene.jpg');
...
%Se generan coordenadas aleatorias de destino para cada receptor (dentro de
%los límites introducidos en la interfaz del simulador
if isempty(filename) 
    %Latitud
    ...
    for rx=1:NUMBER_OF_RECEIVERS,
        ...
        coordinates_start=num2str(39.48044,10)+","+num2str(-6.34482,10);
        coordinates_end=num2str(39.48052,10)+","+num2str(-6.34476,10);
        mode=0;
    ...
    users.latOut=rx_coordinates(:,1);
    users.lonOut=rx_coordinates(:,2);
...
for t=1:step:period+1  %segundos
    for rx=1:NUMBER_OF_RECEIVERS,
    ...
    rx_gain = 2.1;
    sinr_data = sinr(receivers, [uma_transmitters umi_transmitters], ...
        'PropagationModel', PROPAGATION_MODEL, ...
        'ReceiverGain', rx_gain);
    
    txt={};  %Creamos el cell array de etiquetas
    for i=1:NUMBER_OF_RECEIVERS,
        txt(i)={['RX' num2str(i) ': ' num2str(sinr_data(i))]};
    end
    %Represento los valores de sinr de cada receptor
    figure('Color', 'w'); plot(receivers_longitudes, receivers_latitudes, 'ks','MarkerSize',2,'MarkerFaceColor','r');
    ...
    text(receivers_longitudes,receivers_latitudes,txt,'HorizontalAlignment','center','FontSize',12);
